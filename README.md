...
本应用暂未购买代码签名证书，所以首次打开时系统会提示"未知发布者"。这**不代表软件有问题**，是所有未签名应用的通用提示，按下面操作放行即可：

**Windows**：双击安装包后若弹出蓝色窗口「Microsoft Defender SmartScreen 已阻止…」：

1. 点窗口里的「**更多信息**」（More info）
2. 出现「**仍要运行**」（Run anyway）按钮，点它即可正常安装

**macOS**：若提示"无法打开，因为无法验证开发者"：

1. 打开「系统设置 → 隐私与安全性」
2. 拉到底部，找到被拦截的 frp_desktop，点「**仍要打开**」
3. 或者在「访达」里右键应用图标 →「打开」→ 再点「打开」

**Linux AppImage**：下载后需要加可执行权限：`chmod +x frp_desktop_*.AppImage`
